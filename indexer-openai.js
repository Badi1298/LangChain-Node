...
async function runIndexer() {
	...
	try {
		...
		if (!existingIndexes.indexes?.some((index) => index.name === INDEX_NAME)) {
			...
			// Optional: Check if existing index dimension matches VECTOR_DIMENSION
			const desc = await pinecone.describeIndex(INDEX_NAME);
			if (desc.dimension !== VECTOR_DIMENSION) {
				console.error(
					`Error: Index dimension (<span class="math-inline">\{desc\.dimension\}\) does not match OpenAI model dimension \(</span>{VECTOR_DIMENSION}). Please delete and recreate the index or use a matching model.`
				);
				process.exit(1);
			}
		}
